...
export class DojoTxBuilderBase {
     ...
     */
    addOutputs(outputs: (DojoCreateTxOutputApi | DojoCreatingTxOutputApi | DojoTxBuilderOutputApi)[]): void {
     ...
     */
    addChangeOutputs(outputs: (DojoCreateTxOutputApi | DojoCreatingTxOutputApi | DojoTxBuilderOutputApi)[]): void {
        for (const output of outputs) {
            const o: DojoTxBuilderOutputApi = {
                satoshis: output.satoshis,
                script: output.script,
    ...
    validate(noThrow = false): { ok: boolean, error: CwiError | undefined } {
        ...
        try {
            if (!(this?.inputs.length > 0) || !(this?.outputs.length > 0))
                throw new ERR_DOJO_CREATE_TX_EMPTY()

            for (const o of this.outputs) this.validateOutput(o)

        } catch (err) {
            if (noThrow)
                result = { ok: false, error: err as CwiError }
            else
                throw err
        }
