...
export async function processIncomingTransaction (
  ...
  let derivationPrefix: string | undefined
  ...
  if (protocol === '3241645161d8') { // SABPPP peer-to-peer payment protocol
    ...
    for (const out of ptx.outputs) {
      ...
      if (derivationPrefix && out.derivationPrefix && derivationPrefix !== out.derivationPrefix) { throw new ERR_INTERNAL('outputs do not share common derivationPrefix') }
      ...
      if (tx.outputs[out.vout].script.toHex() !== expectedScript) { throw new ERR_DOJO_INVALID_SCRIPT('Transaction output script invalid') }

      if (tx.outputs[out.vout].satoshis !== out.satoshis) { throw new ERR_DOJO_INVALID_SATOSHIS() }
      amount += (out.satoshis || 0)

      if (senderIdentityKey && out.senderIdentityKey && senderIdentityKey !== out.senderIdentityKey) { throw new ERR_INTERNAL('outputs do not share common senderIdentityKey') }
